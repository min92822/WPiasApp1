...
class DoctorAnswerDetailAdapter(var arr : ArrayList<MyAnswerCaseInfo>, var answerInfo : MyAnswerInfo) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.questionHistroy.text = "${toDate(answerInfo.burndate, arr[position].casedate)} 일차"
        ...
        when (arr[position].casestatus) {
            ...
            "A" -> {
                holder.questionAnswerStatus.text = "답변완료"
                holder.questionAnswerStatus.setTextColor(ContextCompat.getColor(context!!, R.color.warm_grey))
                holder.questionAnswerStatus.setBackgroundResource(R.drawable.answer_stroke)
            }
        }
        ...
        holder.questionDetailWrapper.setOnClickListener {
            context!!.startActivity(
                Intent(context!!, DoctorAnsweredCheckActivity::class.java)
                    .putExtra("patientCase", arr[position])
                    .putExtra("answerInfo", answerInfo)
                    .setFlags(
                        Intent.FLAG_ACTIVITY_SINGLE_TOP))
        }

        (context as DoctorAnsweredDetailActivity).Loading((context as DoctorAnsweredDetailActivity).ProgressBar, (context as DoctorAnsweredDetailActivity).ProgressBg, false)
